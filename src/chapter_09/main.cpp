...
public:
	...
	constexpr R base() const& requires std::copy_constructible<R> { return base_; }
...
struct replicate_sentinel {
	...
	constexpr replicate_sentinel(base end)
			:end_{end} { }
...
public:
	...
	constexpr auto size() const requires std::ranges::sized_range<R const> { return count_*std::ranges::size(base_); }

	constexpr auto size() requires std::ranges::sized_range<R> { return count_*std::ranges::size(base_); }
};
