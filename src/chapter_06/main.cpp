...
struct wrapper {
  ...
  bool operator==(std::string_view str)requires std::is_convertible_v<T, std::string_view>
  {
	  return value==str;
  }
};
...
struct wrapper {
  ...
  wrapper(T const& v)requires std::is_copy_constructible_v<T>
		  :value(v) { }
};
}

namespace n632a {
void handle(int v) { /* do something else */ }
